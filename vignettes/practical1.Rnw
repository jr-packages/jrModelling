...
\begin{enumerate}
 \item We conducted an experiment and collected the data in the tables above. This 
 data set isn't paired.\sidenote{I intentionally didn't make the data
...
\begin{enumerate}
\item Input the data into R\sidenote{Here I would suggest input the data into Excel and using \cc{read\_csv}}. Combine the two data sets into a single data frame.
...
\item The formula for calculating the residuals\sidenote{These residuals are
    called Pearson residuals. Hint: use \cc{str(m)} to extract the residuals.} is given by
  \[
